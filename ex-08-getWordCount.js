...
console.log("===== Ex08 getWordCount() =====")

// entra (pasa) un string of text
// convierte string a array
// compara cada palabra con el resto para definir palabras unicas
// array de palabras unicas
// objeto de palabras unicas como propiedades
// comparar cada palabrapropiedad contra cada palabra del texto
// ir contando/sumando con += cada nueva palabra
// cada conteo entra como valor de la propiedad del objeto 
// retorna un OBJETO con propiedades=numero de veces que se repite una palabra
// el conteo debe ser sin case-sensitive
...
function getWordCount(textoAEvaluar) {
	let finalObject = {}
	let primerArray = textoAEvaluar.split(' ')
	console.log(primerArray.length)
	let arrayPalabrasUnicas = []
	let nuevaPalabra = ''
	for ( i = 0 ; i < (primerArray.length-1) ; i++ ) {
		for ( index = 0 ; index < (primerArray.length-1) ; index++ ) {
			if ( primerArray[i] === primerArray[index]) {
				nuevaPalabra = primerArray[i]
			}
		}
